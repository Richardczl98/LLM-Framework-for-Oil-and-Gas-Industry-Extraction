import unittest
from eval.llm_response_parser import LLMResponses
...
class TestLLMResponses(unittest.TestCase):
    ...
    response = LLMResponses()

    def test_separate_num_and_unit(self):
        input = "10Kg"
        want = '10 Kg'
        assert want == self.response._separate_num_and_unit(input)

    def test_parse_same_llm_output(self):
        for field in openai_response:
            for resp in openai_response[field]:
                self.response.parse_same_llm_output(resp, field)
